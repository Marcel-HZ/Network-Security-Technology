...
// 将 MD5 结果转换为字符串表示
string ModifiedMD5::ToString() {
    string strResult;
    for(int i = 0; i < 4; ++i) {
        strResult += DecimalToHex(htobe32(state[i]));
    }
    return strResult;
}

// MD5 变换
void ModifiedMD5::Transform(const vector<uint8_t> block) {
    uint32_t a = state[0], b = state[1], c = state[2], d = state[3];
    vector<uint32_t> x = DivideBlock(block);
